...
import { collection, addDoc, serverTimestamp, doc, setDoc } from "./firebase.js";
...
const COLLEGES = [
 ...
 {id:255,state:'Haryana',name:'Chaudhary Bansi Lal University',city:'Bhiwani',type:'State',estd:2014,rating:4.1,courses:['BA','BSc','BCom','BCA','BBA','MA','MSc','MBA','PhD'],page:'cblubhiwani.html',fees:'₹20,000 – ₹80,000 / yr',intake:5000,accreditation:'UGC Recognized',isUniversity:true},
 {id:256,state:'Punjab',name:'CGC University',city:'Mohali',type:'Private',estd:2025,rating:4.6,courses:['BTech','BCA','BBA','MBA','MTech','MCA','PhD'],page:'cgcuniversity.html',fees:'₹80,000 – ₹2,50,000 / yr',intake:10000,accreditation:'NAAC A+',isUniversity:true},
 {id:257,state:'Haryana',name:'Chaudhary Ranbir Singh University',city:'Jind',type:'State',estd:2014,rating:4.2,courses:['BA','BSc','BCom','BBA','BCA','MA','MSc','MEd','PhD'],page:'crsujind.html',fees:'₹15,000 – ₹70,000 / yr',intake:4000,accreditation:'UGC 2(f) & 12(B)',isUniversity:true},
 {id:258,state:'Haryana',name:'Sushant University',city:'Gurugram',type:'Private',estd:2012,rating:4.3,courses:['BTech','BArch','BBA','BCA','MBA','MTech','Design','Law','PhD'],page:'sushantuniversity.html',fees:'₹1,00,000 – ₹4,00,000 / yr',intake:3000,accreditation:'UGC Recognized',isUniversity:true},
  // ==================== HARYANA ====================
...
async function saveUserToFirestore(name, email) {
  try {
    ...
    const loginData = {
      ...
      loginDay   : days[now.getDay()],
      loginDate  : now.getDate(),
      loginMonth : months[now.getMonth()],
      loginYear  : now.getFullYear(),
      loginHour  : `${hours}:${mins} ${ampm}`
    };

    // 1. loginHistory mein har baar naya record
    
  const docId = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}_${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}_${name.replace(/\s+/g,"_")}`;
  await setDoc(doc(db, "loginHistory", docId), loginData);

    // 2. users mein unique record (update hoga)
    const userId = email.replace(/[@.]/g, "_");
    await setDoc(doc(db, "users", userId), {
      name        : name,
      email       : email,
      lastLogin   : serverTimestamp(),
      lastLoginDay: days[now.getDay()]
    }, { merge: true });
